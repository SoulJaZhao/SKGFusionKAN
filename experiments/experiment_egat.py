...
class TwoLayerGAT(nn.Module):
    def __init__(self, in_feats, hidden_feats, out_feats, dropout):
        ...
        self.gat1 = EGATConv(in_node_feats=in_feats, in_edge_feats=in_feats, out_node_feats=128, out_edge_feats=out_feats, num_heads=1)
        self.gat2 = EGATConv(in_node_feats=128, in_edge_feats=out_feats, out_node_feats=out_feats, out_edge_feats=out_feats, num_heads=1)

    def forward(self, g, nfeats, efeats):
        # Apply first GAT layer
        h, e = self.gat1(g, nfeats, efeats)
        h = th.relu(h)
        e = th.relu(e)
...
epochs = 3000
binary_best_model_file_path = f'./binary_model/EGAT_{dataset}_best_model.pth'
binary_report_file_path = f'./binary_reports/EGAT_{dataset}_report.json'
binary_test_pred_file_path = f'./binary_predictions/EGAT_{dataset}_test_pred.pth'

multiclass_best_model_file_path = f'./model/EGAT_{dataset}_best_model.pth'
multiclass_report_file_path = f'./reports/EGAT_{dataset}_report.json'
multiclass_test_pred_file_path = f'./predictions/EGAT_{dataset}_test_pred.pth'
